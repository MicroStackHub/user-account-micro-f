...
const sidebarConfig = [
  {
    label: 'Dashboard',
    icon: (active: boolean) => (
      <svg className={`w-6 h-6 flex-shrink-0 transition duration-75 ${active ? 'text-orange-600' : 'text-gray-500 group-hover:text-gray-900'}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
  ...
  },
  {
    ...
    key: 'profile',
    submenu: [
      { label: 'My Profile', route: '/profile', key: 'my-profile' },
      { label: 'Set Refund Payment', route: '/refund', key: 'refund' },
      { label: 'Set GST Number', route: '/gst', key: 'gst' },
    ],
  },
  {
                ...
                <li key={item.key}>
                  {item.submenu ? (
                    <button
                      ...
                      className={`text-base font-normal rounded-lg flex items-center p-2 hover:bg-gray-100 group w-full ${activeItem === item.key ? 'bg-orange-50 text-orange-600' : 'text-gray-900'}`}
                    >
                      {item.icon(activeItem === item.key)}
                      <span className={`ml-3 flex-1 whitespace-nowrap ${isCompact ? 'hidden' : 'block'}`}>{item.label}</span>
                      {!isCompact && (
                        <svg className={`w-6 h-6 ${activeSubmenu === item.key ? 'transform rotate-180' : ''}`} fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                          <path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd"></path>
                        </svg>
                      )}
                      {isCompact && activeItem === item.key && (
                    ...
                    <Link
                      to={item.route}
                      className={`text-base font-normal rounded-lg flex items-center p-2 hover:bg-gray-100 group ${activeItem === item.key ? 'bg-orange-50 text-orange-600' : 'text-gray-900'}`}
                    >
                      {item.icon(activeItem === item.key)}
                      <span className={`ml-3 flex-1 whitespace-nowrap ${isCompact ? 'hidden' : 'block'}`}>{item.label}</span>
                      {item.badge && (
                  ...
                  {item.submenu && activeSubmenu === item.key && !isCompact && (
                    <ul className="py-2 space-y-2 pl-6">
                      {item.submenu.map((subItem) => (
                        <li key={subItem.key}>
                          <Link
                            to={subItem.route}
                            className={`text-base text-gray-900 font-normal rounded-lg hover:bg-gray-100 flex items-center p-2 group ${activeItem === subItem.key ? 'bg-orange-50 text-orange-600' : ''}`}
                          >
